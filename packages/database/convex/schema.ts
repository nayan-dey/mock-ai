...
export default defineSchema({
  ...
  batches: defineTable({
    ...
    monthlyFee: v.optional(v.number()), // Recurring monthly fee amount
    enrollmentFee: v.optional(v.number()), // One-time fee on joining
  ...
  users: defineTable({
    ...
    enrolledAt: v.optional(v.number()), // Timestamp when student joined batch
  ...
  fees: defineTable({
    ...
    description: v.optional(v.string()),
    isAutoGenerated: v.optional(v.boolean()), // True if created by system
    organizationId: v.id("organizations"),
    createdBy: v.id("users"),
    createdAt: v.number(),
  })
  ...
  // Fee queries/disputes from students
  feeQueries: defineTable({
    feeId: v.id("fees"),
    studentId: v.id("users"),
    organizationId: v.id("organizations"),
    type: v.union(
  ...
  // Fee query messages for conversation thread
  feeQueryMessages: defineTable({
